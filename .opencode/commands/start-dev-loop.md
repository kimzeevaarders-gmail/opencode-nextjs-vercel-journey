...
7. If the reviewer approves, commit the changes, push `main`, post the reviewer approval note to the GitHub issue, close the issue, and verify that Vercel starts a production deployment.
8. Summarize the issue number, files changed, review result, commit SHA, issue-close status, and deploy status.
